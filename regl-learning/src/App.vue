 ...
 */
    function cpuReduce (data: string|any[], op: { (a: any, b: any): number; (arg0: any, arg1: any): any }) {
...
*/
    function gpuReduceCreate (data: string|any[], op: string) {
    ...
    void main () {
      ...
      gl_Position = vec4(1.0 - 2.0 * position, 0, 1);
        ...
        uniforms: {
          tex: regl.prop<{inTex:WebGLTexture}, 'inTex'>('inTex'),
          rcpDim: regl.prop<{'rcpDim':number}, 'rcpDim'>('rcpDim') // reciprocal texture dimensions.
        },
        framebuffer: regl.prop<{'outFbo':any}, 'outFbo'>('outFbo'),
    ...
    function createParagraph (elem: string, text: string) {
    ...
    function profile (gpu: boolean) {
      ...
      function loop () {
        ...
        var cmd = gpu
          ? () => gpuReduce()
          : () => cpuReduce(data, (a: number, b: number) => Math.max(a, b))
